...
- 可自定义源语言、默认目标语言和自动反向目标语言；语言列表来自当前系统的 Apple Translation
- 界面完整支持简体中文与英文，并跟随 macOS 的应用语言设置
- 非激活式 `NSPanel`，支持多显示器、Space 和全屏应用
- 新请求取消旧请求，过期结果不会覆盖当前结果
- 翻译窗口默认尺寸为 640 × 500，可拖动右下角调整大小并自动记忆
- 原文和译文均可单独复制；不保存历史、不记录原文或译文
...
- [ ] 检查深色模式和浅色模式
- [ ] 点击“复制译文”并验证轻量成功反馈
- [ ] 分别点击“复制原文”和“复制译文”，确认内容正确
- [ ] 拖动翻译窗口右下角调整大小，重新翻译后确认尺寸被记住
- [ ] 在“设置”中更改快捷键、源语言和目标语言并验证生效
- [ ] 确认系统未请求屏幕录制权限
- [ ] 确认系统未请求输入监控权限
- [ ] 确认应用不显示 Dock 图标
- [ ] 确认悬浮面板没有让原应用明显失焦
